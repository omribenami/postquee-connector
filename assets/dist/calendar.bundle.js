...
const CalendarProvider = ({ children }) => {
    ...
    return (react.createElement(CalendarContext.Provider, { value: { posts, integrations, isLoading, error, refresh, deletePost, reschedulePost } }, children));
};
 ...
 */
const DndProvider_DndProvider = ({ children }) => {
 ...
 */
const PostCard = ({ post, variant = 'week', onDelete, onEdit }) => {
        ...
        react.createElement("div", { className: "flex items-center gap-2 mb-1" },
            ...
            react.createElement("div", { className: "ml-auto opacity-0 group-hover:opacity-100 flex gap-1 transition-opacity" },
                onEdit && (react.createElement("button", { onClick: handleEdit, className: "p-1 hover:bg-newBoxHover rounded", title: "Edit post" },
                    react.createElement("svg", { className: "w-3 h-3 text-textItemBlur", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24" },
...
const CalendarSlot_DND_TYPE = 'POST';
/**
 * Droppable Calendar Time Slot
 */
const CalendarSlot = ({ date, hour, posts, onDrop, onCreate, onDeletePost, onEditPost, variant = 'week', className = '', }) => {
    // Create the exact datetime for this slot
    const slotDateTime = hour !== undefined ? date.hour(hour).minute(0).second(0) : date;
    const isPast = slotDateTime.isBefore(dayjs_min_default()());
    const isToday = slotDateTime.isSame(dayjs_min_default()(), 'day');
    const [{ isOver, canDrop }, drop] = useDrop(() => ({
        accept: CalendarSlot_DND_TYPE,
    ...
    const getBorderColor = () => {
        if (isOver && canDrop)
            return 'border-btnPrimary';
        if (canDrop)
            return 'border-newBorder';
        return 'border-newBorder';
    };
    return (react.createElement("div", { ref: drop, onClick: handleClick, className: `
        ${className}
      ...
      ` },
        !isPast && posts.length === 0 && (react.createElement("div", { className: "hidden group-hover:flex items-center justify-center h-full text-btnPrimary text-2xl font-bold" }, "+")),
        react.createElement("div", { className: "space-y-1" }, posts.map((post) => (react.createElement(PostCard, { key: post.id, post: post, variant: variant, onDelete: onDeletePost, onEdit: onEditPost }))))));
};
...
const WeekView = ({ onCreate, onEdit }) => {
    ...
    const handleDrop = async (post, newDate) => {
    ...
    return (react.createElement("div", { className: "h-full overflow-auto" },
        react.createElement("div", { className: "grid gap-0", style: { gridTemplateColumns: '80px repeat(7, minmax(0, 1fr))' } },
            react.createElement("div", { className: "sticky top-0 bg-newBgColor border-b border-newBorder p-2 z-10" }),
            weekDays.map((day) => (react.createElement("div", { key: day.format('YYYY-MM-DD'), className: "sticky top-0 bg-newBgColor border-b border-l border-newBorder p-3 text-center z-10" },
            ...
            HOURS.map((hour) => (react.createElement(react.Fragment, { key: hour },
                ...
                weekDays.map((day) => {
                    ...
                    return (react.createElement(CalendarSlot, { key: `${day.format('YYYY-MM-DD')}-${hour}`, date: day, hour: hour, posts: slotPosts, onDrop: handleDrop, onCreate: onCreate, onDeletePost: handleDelete, onEditPost: onEdit, variant: "week", className: "border-l border-b min-h-[60px] p-1" }));
                })))))));
};
...
dayjs_min_default().extend((isoWeek_default()));
const MonthView = ({ onCreate, onEdit }) => {
    const { currentDate } = useCalendarStore();
    const { posts, deletePost, reschedulePost } = useCalendarContext();
    ...
    const handleDrop = async (post, newDate) => {
        ...
        const originalTime = dayjs_min_default()(post.publishDate);
    ...
    return (react.createElement("div", { className: "h-full overflow-auto p-4" },
        ...
        react.createElement("div", { className: "grid grid-cols-7 gap-1", style: { gridAutoRows: 'minmax(120px, 1fr)' } }, calendarDays.map((day) => {
            const dayPosts = getPostsForDay(day);
            const isToday = day.isSame(dayjs_min_default()(), 'day');
            const isCurrentMonth = day.month() === monthStart.month();
            return (react.createElement("div", { key: day.format('YYYY-MM-DD'), className: `rounded-lg ${!isCurrentMonth ? 'opacity-40' : ''}` },
                ...
                react.createElement(CalendarSlot, { date: day, posts: dayPosts, onDrop: handleDrop, onCreate: onCreate, onDeletePost: handleDelete, onEditPost: onEdit, variant: "month", className: "px-2 pb-2 min-h-[80px]" }),
                dayPosts.length > 3 && (react.createElement("div", { className: "text-xs text-btnPrimary font-medium text-center px-2 pb-2" },
                    "+",
                    dayPosts.length - 3,
                    " more"))));
        }))));
...
const DayView = ({ onCreate, onEdit }) => {
    ...
    return (react.createElement("div", { className: "h-full overflow-auto" },
        react.createElement("div", { className: "max-w-4xl mx-auto p-4" }, DayView_HOURS.map((hour) => {
            ...
            return (react.createElement("div", { key: hour, className: "flex border-b border-newBorder" },
                ...
                react.createElement(CalendarSlot, { date: day, hour: hour, posts: hourPosts, onDrop: handleDrop, onCreate: onCreate, onDeletePost: handleDelete, onEditPost: onEdit, variant: "day", className: "flex-1 p-4 min-h-[100px]" })));
        }))));
};
...
const CalendarContent = ({ onCreatePost, onEditPost }) => {
    ...
    switch (view) {
        case 'day':
            return react.createElement(DayView, { onCreate: onCreatePost, onEdit: onEditPost });
        case 'week':
            return react.createElement(WeekView, { onCreate: onCreatePost, onEdit: onEditPost });
        case 'month':
            return react.createElement(MonthView, { onCreate: onCreatePost, onEdit: onEditPost });
        default:
            return null;
    }
};
const PostModal = ({ date, post, onClose }) => {
    if (!date)
        return null;
    return (react.createElement("div", { className: "fixed inset-0 bg-newBackdrop flex items-center justify-center z-50", onClick: onClose },
        react.createElement("div", { className: "bg-newSettings rounded-lg border border-newBorder p-6 max-w-2xl w-full mx-4", onClick: (e) => e.stopPropagation() },
            react.createElement("div", { className: "flex items-center justify-between mb-4" },
                react.createElement("h2", { className: "text-xl font-bold text-newTextColor" }, post ? 'Edit Post' : 'Create Post'),
                react.createElement("button", { onClick: onClose, className: "text-textItemBlur hover:text-newTextColor" },
                    react.createElement("svg", { className: "w-6 h-6", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24" },
                        react.createElement("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M6 18L18 6M6 6l12 12" })))),
            react.createElement("div", { className: "text-textItemBlur mb-6" },
                react.createElement("p", { className: "mb-2" },
                    "Scheduled for: ",
                    react.createElement("span", { className: "text-newTextColor font-semibold" }, date.format('MMMM D, YYYY [at] HH:mm'))),
                react.createElement("p", { className: "text-sm text-yellow-400" }, "\uD83D\uDEA7 Full post creator with TipTap editor, AI assistant, and channel settings coming in Phase 4-6!")),
            react.createElement("div", { className: "bg-newBgColor rounded p-4 text-center text-textItemBlur" },
                react.createElement("p", null, "For now, you can:"),
                react.createElement("ul", { className: "mt-2 space-y-1 text-sm" },
                    react.createElement("li", null, "\u2705 Drag & drop posts to reschedule them"),
                    react.createElement("li", null, "\u2705 Delete posts (hover over post \u2192 trash icon)"),
                    react.createElement("li", null, "\u2705 Navigate calendar views (Day/Week/Month)"))),
            react.createElement("div", { className: "mt-6 flex justify-end" },
                react.createElement("button", { onClick: onClose, className: "px-4 py-2 bg-btnPrimary text-white rounded hover:bg-btnPrimaryHover transition-colors" }, "Close")))));
};
/**
...
const CalendarApp = () => {
    ...
    const handleEditPost = (post) => {
        setModalDate(dayjs_min_default()(post.publishDate));
        setEditingPost(post);
    };
    ...
    return (react.createElement(DndProvider_DndProvider, null,
        react.createElement(CalendarProvider, null,
            ...
            react.createElement(PostModal, { date: modalDate, post: editingPost, onClose: handleCloseModal }))));
};
